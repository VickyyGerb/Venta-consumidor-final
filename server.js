  ...
  async function basic(page, correo, contraseña, cliente, listaPrecio, producto, cantProducto, valorBonificacion, productos, cantidades, bonificaciones, nuevoProductoUpper, nombreNuevoProducto, tipoNuevoProducto, ivaNuevoProducto, contableVenta, contableCompra, categoriaNuevoProducto, codigoProveedor, codigoBarra, productoLibreUpper, descripcionProductoLibre, cantidadProductoLibre, precioProductoLibre, bonificacionProductoLibre, productoNormalUpper, precioNuevoProducto, cantProductoNuevo, tipoProductoLibre, bonificacionProductoNuevo, condicionPago, cantidadCuotas, cantTotalProductos, observacion, pagoTarjeta, tipoTarjeta, numeroTarjeta, montoTarjeta, pagoTransferencia, cuentaTransferencia, montoTransferencia, grupoCaja, descuento) {
    ...
    if (descuento && descuento.trim() !== "" && descuento !== "0") {
      const descuentoLimpiado = descuento.trim().replace('%', '');
      
      if (!isNaN(descuentoLimpiado) && descuentoLimpiado !== "") {
          if (descuento.includes('%')) {
              console.log("Es un descuento porcentual:", descuentoLimpiado + "%");
              await page.locator("#DescuentoComprobanteUnificado").fill(descuentoLimpiado);
              await page.waitForTimeout(2000);
              await page.keyboard.press("Enter");
          } else {
              console.log("Es un descuento en monto fijo:", descuentoLimpiado);
              await page.getByRole('textbox', { name: 'Descuento $' }).fill(descuentoLimpiado);
              await page.waitForTimeout(2000);
              await page.keyboard.press("Enter");
          }
      } else {
          console.log("El descuento no es un número válido");
      }
    }
    ...
    console.log('precioTotal:', precioTotal, 'tipo:', typeof precioTotal, 'truthy?', !!precioTotal);
    console.log('montoTarjeta:', montoTarjeta, 'tipo:', typeof montoTarjeta, 'truthy?', !!montoTarjeta);
    console.log('montoTransferencia:', montoTransferencia, 'tipo:', typeof montoTransferencia, 'truthy?', !!montoTransferencia);
    console.log('Condición completa:', !!(precioTotal && (montoTarjeta || montoTransferencia)));

    const precioTotalValido = precioTotal !== undefined && precioTotal !== null;
    const montoTarjetaValido = montoTarjeta !== undefined && montoTarjeta !== null;
    const montoTransferenciaValido = montoTransferencia !== undefined && montoTransferencia !== null;

    if (precioTotalValido && (montoTarjetaValido || montoTransferenciaValido)) {
      console.log('✅ CONDICIÓN CUMPLIDA - Ejecutando validación de pagos');
      ...
      try {
        ...
        try {
          const efectivoElement = await page.locator('#Recibo_Efectivo').first();
          await efectivoElement.waitFor({ state: 'visible', timeout: 5000 });
          const efectivoValue = await efectivoElement.inputValue();
          console.log(`Efectivo ajustado: $${efectivoValue}`);
        } catch (error) {
          console.log('No se pudo obtener el efectivo:', error.message);
        }
      ...
      console.log('❌ CONDICIÓN NO CUMPLIDA - Razones:');
      if (!precioTotalValido) console.log('- precioTotal es undefined o null');
      if (!montoTarjetaValido && !montoTransferenciaValido) console.log('- ambos montos son undefined o null');
      
      console.log('--- Valores actuales ---');
      console.log('precioTotalValido:', precioTotalValido);
      console.log('montoTarjetaValido:', montoTarjetaValido);
      console.log('montoTransferenciaValido:', montoTransferenciaValido);
      console.log('------------------------');
    }
      }
  ...
  if (grupoCaja && grupoCaja.trim() !== "") {
    try {
      await page.locator('#CajaGrupoId_chosen a').click();
      await page.waitForTimeout(1000);
      await page.keyboard.type(grupoCaja);
      await page.waitForTimeout(2000);
      await page.keyboard.press('Enter');
      await page.waitForTimeout(1000);
      console.log(`Grupo de caja seleccionado: ${grupoCaja}`);
    } catch (error) {
      console.log('Error al seleccionar grupo de caja:', error.message);
    }
  }
  ...
  app.post("/login-basic", async (req, res) => {
    const { correo, contraseña, cliente, listaPrecio, producto, cantProducto, 
            ...
            bonificacionProductoLibre, productoNormal, precioNuevoProducto, cantProductoNuevo, tipoProductoLibre, bonificacionProductoNuevo, condicionPago, cantidadCuotas, observacion, pagoTarjeta, tipoTarjeta, numeroTarjeta, montoTarjeta, pagoTransferencia, cuentaTransferencia, montoTransferencia, grupoCaja, descuento } = req.body;
